...
async function main() {
  ...
  function onMutation() {
    ...
    if (settings.blockAIStudio) {
      const AIStudioLink = body?.querySelector(selectors.AIStudio);
      hide(AIStudioLink);
    }
    if (settings.blockFooter) {
      const footer = body?.querySelector(selectors.footer);
      hide(footer);
    }
    if (settings.blockThreads) {
      ...
      anchorTags.forEach((anchor) => {
        ...
        // Check if the 'xmt' parameter exists
        if (url.searchParams.has("xmt")) {
          // Perform your desired action here
          console.log("Anchor with xmt parameter found:", anchor);
          hide(anchor);
    ...
    const blockAIStudio = path.includes(urls.AIStudio) && settings.blockExplore;
    if (blockExploreScreen) {
      const main = body?.querySelector(selectors.main);
      hide(main);
    }
